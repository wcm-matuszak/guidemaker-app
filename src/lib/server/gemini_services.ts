...
export async function generateItinerary(places: any[], cityName: string, full_address: string) {
    ...
    - Structure the itinerary in three parts using <h2> tags:
      ...
      * Evening (17:00-22:00)
    - For each time block, recommend 2-3 suitable attractions
    - When mentioning a place for the first time, format it as: <a href="{googleMapsUri}" target="_blank" rel="noopener noreferrer">{displayName.text}</a>
